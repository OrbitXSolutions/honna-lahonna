...
const bioSchema = serviceProviderSchema.pick({
  bio: true,
});
...
export function ProfileBioField({ bio, disableEdit = false }: Props) {
  ...
  const { execute, isPending, result } = useAction(profileUpdateAction, {
    ...
    onError: ({ error }) => {
      ...
      const message =
        error?.validationErrors?.bio?._errors?.join(",") ?? error?.serverError;
  ...
  const save = () => {
    const validation = bioSchema.safeParse({ bio: value });
    if (validation.error) {
    ...
    <>
      ...
      <div className={"p-4 text-gray-600 text-sm bg-white rounded-2xl"}>
        {isEdit ? (
          ...
          <span>{value || "ادخل سيرتك الذاتية"}</span>
        )}
      </div>
    </>
  );
